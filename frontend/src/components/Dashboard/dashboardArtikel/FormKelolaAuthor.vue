<template>
  <div class="form-container">
    ...
    <div class="existing-authors">
      ...
      <ul>
        <li v-for="(author, index) in authors" :key="author.id" class="author-item">
          <span v-if="!author.editing">{{ author.name }}</span>
          <input
            v-else
            v-model="author.editName"
          ...
          <div class="action-buttons">
            ...
            <button class="btn btn-save" @click="updateAuthor(index)" v-else :disabled="author.updating">
              {{ author.updating ? 'Menyimpan...' : 'Simpan' }}
            </button>
            <button class="btn btn-cancel" @click="cancelEdit(index)" v-if="author.editing">Batal</button>
            <button class="btn btn-delete" @click="deleteAuthor(index)" v-if="!author.editing" :disabled="author.deleting">
              {{ author.deleting ? 'Menghapus...' : 'Hapus' }}
            </button>
          </div>
        </li>
      </ul>
    ...
    <div v-if="showDeleteModal" class="modal-overlay" @click="closeDeleteModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h3>Konfirmasi Hapus Author</h3>
          <button class="modal-close" @click="closeDeleteModal">×</button>
        </div>
        <div class="modal-body">
          <div class="warning-icon">⚠️</div>
          <p>Apakah Anda yakin ingin menghapus author <strong>"{{ deleteTarget?.name }}"</strong>?</p>
          <div v-if="deleteTarget?.articleCount > 0" class="delete-warning">
            <p><strong>Peringatan:</strong> Author ini memiliki {{ deleteTarget.articleCount }} artikel yang masih terkait.</p>
            <p>Untuk menghapus author ini, Anda perlu:</p>
            <ul>
              <li>Unpublish artikel yang terkait, atau</li>
              <li>Ganti author artikel ke author lain</li>
            </ul>
          </div>
        </div>
    ...
    <div v-if="showErrorModal" class="modal-overlay" @click="closeErrorModal">
      <div class="modal-content error-modal" @click.stop>
        <div class="modal-header error-header">
          <h3>❌ Tidak Dapat Menghapus Author</h3>
          <button class="modal-close" @click="closeErrorModal">×</button>
        </div>
        <div class="modal-body">
          <div class="error-icon-large">🚫</div>
          <p class="error-title">Author <strong>"{{ errorModalData?.authorName }}"</strong> tidak dapat dihapus!</p>
          <div class="error-explanation">
            ...
            <div class="error-steps">
              ...
              <ol>
                ...
                <li>Cari artikel yang menggunakan author ini</li>
                <li>Unpublish artikel tersebut, ATAU</li>
                <li>Ganti author artikel ke author lain</li>
                <li>Setelah itu, coba hapus author lagi</li>
              </ol>
            </div>
          </div>
        </div>
...
const buildRelatedArticleMessage = (count) =>
  `Tidak dapat menghapus author yang masih memiliki ${count} artikel published. ` +
...
const handleApiError = (error, action = 'melakukan operasi') => {
  ...
  if (error?.response?.status === 400) {
    ...
    if (
      ...
      errorSuggestion.value = 'Silakan unpublish artikel terlebih dahulu atau ganti author artikel ke author lain sebelum menghapus author ini.';
    } else if (message.includes('sudah ada')) {
      errorSuggestion.value = 'Coba gunakan nama yang berbeda untuk author Anda.';
    }
  } else if (error?.response?.status === 404) {
    errorMessage.value = 'Author tidak ditemukan. Mungkin sudah dihapus sebelumnya.';
...
const showDeleteErrorModal = (authorName, msg) => {
  errorModalData.value = { authorName, message: msg };
...
// Data sync trigger
// =========================
const triggerDataUpdate = (type = 'author') => {
  window.dispatchEvent(new CustomEvent('dataUpdated', { 
    detail: { type, timestamp: Date.now() } 
  }));
  
  localStorage.setItem('authorUpdated', Date.now().toString());
  setTimeout(() => localStorage.removeItem('authorUpdated'), 100);
};
...
const fetchAuthors = async () => {
  try {
    ...
    handleApiError(error, 'memuat data author');
  }
};
...
const handleAddAuthor = async () => {
  ...
  try {
    const response = await axios.post(`${API_BASE}/admin/authors`, {
    ...
    if (response.data.success) {
      ...
      successMessage.value = response.data.message || 'Author berhasil ditambahkan.';
      errorMessage.value = '';
      errorSuggestion.value = '';
      newAuthor.value = '';
      
      triggerDataUpdate('author_added');
    }
  } catch (error) {
    handleApiError(error, 'menambahkan author');
  } finally {
    loading.value = false;
  }
};
...
const cancelEdit = (index) => {
  ...
  author.editName = author.originalName;
  author.updating = false;
  
  errorMessage.value = '';
  errorSuggestion.value = '';
};

const updateAuthor = async (index) => {
  const author = authors.value[index];
  const updatedName = author.editName?.trim();
  ...
  if (updatedName === author.originalName) {
    author.editing = false;
    return;
  }
  ...
  author.updating = true;

  try {
    const response = await axios.put(`${API_BASE}/admin/authors/${author.id}`, {
      name: updatedName
    }, {
      headers: getAuthHeaders()
    });
    ...
    if (response.data.success) {
      const oldName = author.originalName;
      
      author.name = response.data.data.name;
      author.originalName = response.data.data.name;
      author.editName = response.data.data.name;
      author.editing = false;
      
      successMessage.value = response.data.message || 'Author berhasil diperbarui.';
      errorMessage.value = '';
      errorSuggestion.value = '';
      
      console.log(`Author updated: ${oldName} -> ${author.name}`);
      triggerDataUpdate('author_updated');
    }
  } catch (error) {
    handleApiError(error, 'memperbarui author');
...
const deleteAuthor = async (index) => {
  ...
  try {
    const response = await axios.get(`${API_BASE}/admin/authors/${author.id}/articles-count`, {
      headers: getAuthHeaders()
    });
    author.articleCount = response.data?.count || 0;
  } catch (error) {
    console.warn('Could not fetch article count:', error);
    author.articleCount = 0;
  }
  
  deleteTarget.value = { ...author, index };
...
const confirmDelete = async () => {
  ...
  try {
    const response = await axios.delete(`${API_BASE}/admin/authors/${author.id}`, {
      headers: getAuthHeaders()
    });
    
    if (response.data.success) {
      authors.value.splice(index, 1);
      successMessage.value = response.data.message || 'Author berhasil dihapus.';
      errorMessage.value = '';
      errorSuggestion.value = '';
      
      console.log(`Author deleted: ${authorName}`);
      triggerDataUpdate('author_deleted');
      closeDeleteModal();
    }
  } catch (error) {
    ...
    if (error.response) {
      ...
      if (status === 400) {
        // Bad request - author has related articles
        showDeleteErrorModal(authorName, message);
      } else if (status === 404) {
        // Author not found - remove from list
        authors.value.splice(index, 1);
        errorMessage.value = 'Author tidak ditemukan. Mungkin sudah dihapus sebelumnya.';
        errorSuggestion.value = 'Data telah diperbarui.';
      } else if (status === 500) {
        // Server error
        showDeleteErrorModal(authorName, 'Terjadi kesalahan server. Silakan coba lagi nanti.');
      } else {
        // Other errors
        showDeleteErrorModal(authorName, message);
      }
      ...
      showDeleteErrorModal(authorName, 'Tidak dapat terhubung ke server. Periksa koneksi internet Anda.');
    } else {
      // Other error
      showDeleteErrorModal(authorName, 'Terjadi kesalahan tidak terduga. Silakan coba lagi.');
    }
  }
};
...
onMounted(async () => {
  console.log('Author management component mounted, loading authors...');
  await fetchAuthors();
});

onBeforeUnmount(() => {
  localStorage.removeItem('authorUpdated');
});
</script>
